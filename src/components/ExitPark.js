...
export function ExitPark() {
  ...
  const handleParkExit = async (event) => {
    ...
    try {
      ...
      console.log(err);
      setExitError(err.response.data.message);
    }
  };
    ...
    <Container component='main' maxWidth='xs'>
      <Box
        ...
        <Box component='form' onSubmit={handleParkExit} sx={{ mt: 3 }}>
          ...
          <Button onClick={() => navigate('/')} fullWidth variant='contained' color='error'>
            Cancel
          </Button>
        </Box>
        {parkExitMessage && (
          <TableContainer
            ...
            <Table size='medium' aria-label='parking invoice'>
              <TableBody>
                ...
                <TableRow sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                  <TableCell align='left' component='th' scope='row' sx={{ fontWeight: 'bold' }}>
                    Parking time
                  </TableCell>
                  <TableCell sx={{ fontWeight: 'bold' }}>{parkExitMessage.parkTime} min</TableCell>
                </TableRow>
              </TableBody>
            </Table>
          </TableContainer>
